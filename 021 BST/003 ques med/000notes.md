...
---
 ...
 # Q LC538. Convert BST to Greater Tree
...
### **Problem Statement**
Given the `root` of a Binary Search Tree (BST), convert it to a **Greater Tree** such that every original key in the BST is changed to the original key plus the sum of all keys greater than the original key in the BST.
...
### **Intuition**
In a BST, the values are naturally ordered. If we perform a standard **Inorder Traversal** (Left -> Root -> Right), we get values in ascending order. 

However, this problem asks us to add the sum of all **greater** values to each node. This suggests we should process the largest values first. We can achieve this using a **Reverse Inorder Traversal**:
1. Visit the **Right** subtree (largest values).
2. Process the **Root**.
3. Visit the **Left** subtree (smallest values).

By maintaining a running `sum` of all nodes visited so far, we can simply update the current node's value:
...
### **Complexity Analysis**
- **Time Complexity:** $O(N)$, where $N$ is the number of nodes in the tree. We visit each node exactly once.
- **Space Complexity:** $O(H)$, where $H$ is the height of the tree, representing the recursion stack. In the worst case (a skewed tree), $H = N$; in a balanced tree, $H = \log N$.
