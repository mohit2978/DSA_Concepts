...
```
This is a solution for the **Job Sequencing Problem**.
The active code uses the **Correct Greedy Strategy** by sorting based on **Profit (Descending)**.

Here is the explanation of why this works and a critical bug fix you need to make regarding array bounds.

### 1. Why Sort by Profit (Descending)?
The goal is to maximize **Total Profit**, not the number of jobs.

* If you sort by **Deadline (Ascending)** (the commented-out part), you prioritise urgency over money. You might fill your available slots with "cheap" jobs just because they expire soon, leaving no room for a "high-value" job that comes later in the sorted list.
* By sorting by **Profit (Descending)**, you consider the most valuable jobs first. You try your best to fit them into the schedule. If they fit, great! If they don't, you skip them. This guarantees that if a slot is occupied, it is occupied by the most profitable job possible for that slot.

### 2. Why Fill Slots Backwards (`dl` down to 1)?
Once you pick a high-profit job, you want to perform it as **late as possible** (closest to its deadline).

* **Reason:** Doing a job at the last possible moment leaves the earlier time slots free for other jobs that might have tighter deadlines.
* **Example:** Job A (Deadline 5), Job B (Deadline 1).
    * If you do A at Time 1, you block B.
    * If you do A at Time 5, Time 1 is free for B.
